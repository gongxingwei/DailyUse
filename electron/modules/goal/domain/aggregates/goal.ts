...
import type { IGoalCreateDTO, IRecordCreateDTO } from "../../../../../common/modules/goal/types/goal";
...
export class Goal extends AggregateRoot implements IGoal {
   ...
   * 添加记录并更新关键结果
   */
  addRecord(recordCreateDTO: IRecordCreateDTO): Record {
    // 验证关键结果存在
    const keyResult = this._keyResults.find(kr => kr.uuid === recordCreateDTO.keyResultId);
    if (!keyResult) {
      throw new Error(`关键结果不存在: ${recordCreateDTO.keyResultId}`);
    }

    if (recordCreateDTO.value <= 0) {
      throw new Error("记录值必须大于0");
    }

    // 创建记录
    const record = new Record(
      recordCreateDTO.goalUuid,
      recordCreateDTO.keyResultId,
      recordCreateDTO.value,
      TimeUtils.now(),
      undefined, // uuid will be generated
      recordCreateDTO.note
    );

    this._records.push(record);

    // 更新关键结果当前值
    keyResult.addValue(recordCreateDTO.value);
  ...
  updateReview(reviewId: string, updates: {
    name?: string;
    content?: Partial<GoalReview['content']>;
    rating?: GoalReview['rating'];
  }): void {
   ...
   * 获取特定类型的复盘
   */
  getReviewsByType(type: GoalReview['type']): GoalReview[] {
  ...
  getReviewStatistics(): {
    totalReviews: number;
    reviewsByType: { [K in GoalReview['type']]: number };
   ...
   * 从创建数据传输对象创建目标
   */
  static fromCreateDTO(data: IGoalCreateDTO): Goal {
    const goal = new Goal(undefined, data.name, {
      description: data.description,
      color: data.color,
      dirId: data.dirId,
      startTime: data.startTime,
