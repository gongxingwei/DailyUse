...
import { ISessionLogRepository } from '../../domain/repositories/sessionLoggingRepository';

/**
 * SQLite SessionLog 仓库实现
 */
export class SqliteSessionLogRepository implements ISessionLogRepository {
   ...
   * 根据账户ID和时间范围查找会话日志
   */
  async findByAccountIdAndTimeRange(accountId: string, startTime: Date, endTime: Date): Promise<SessionLog[]> {
    const stmt = this.db.prepare(`
      SELECT * FROM session_logs 
   ...
   * 获取账户的登录统计
   */
  async getLoginStats(accountId: string, days: number = 30): Promise<{
    totalLogins: number;
