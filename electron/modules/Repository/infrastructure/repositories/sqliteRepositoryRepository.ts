...
export class SqliteRepositoryRepository implements IRepositoryRepository {
  ...
  async addRepository(repository: Repository): Promise<void> {
    try {
      ...
      db.prepare(
        `INSERT INTO repositories (id, name, path, description, createdAt, updatedAt, relatedGoals) VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).run(
        repository.id,
        repository.name,
        repository.path,
  ...
  async updateRepository(repository: Repository): Promise<void> {
    try {
      const db = await this.getDb();
      db.prepare(
        `UPDATE repositories SET name = ?, path = ?, description = ?, updatedAt = ?, relatedGoals = ? WHERE id = ?`
      ).run(
        repository.name,
        repository.path,
        repository.description,
  ...
  async getRepositoryById(repositoryId: string): Promise<Repository | null> {
    try {
      ...
      const row = db
        ...
        .get(repositoryId) as Record<string, any>;
      if (!row) return null;
  ...
  async findAllRepositories(): Promise<Repository[]> {
    try {
      const db = await this.getDb();
      const rows = db.prepare(`SELECT * FROM repositories`).all() as Record<
        string,
        any
      >[];
