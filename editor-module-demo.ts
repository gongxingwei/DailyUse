...
function createServerComponents() {
  ...
  const documentRepository = {
    create: async (doc: any) => ({ ...doc, uuid: 'doc-123', createdAt: new Date() }),
    findByUuid: async (uuid: string) => ({ uuid, title: 'Sample Doc', content: '# Hello' }),
    search: async (query: any) => [],
  };

  const workspaceRepository = {
    create: async (ws: any) => ({ ...ws, uuid: 'ws-123', createdAt: new Date() }),
    findByUuid: async (uuid: string) => ({ uuid, name: 'Sample Workspace' }),
  };
...
function createClientComponents() {
  ...
  const documentListAdapter = {
    setDocuments: function (docs: any[]) {
      console.log(`📋 Document list updated: ${docs.length} documents`);
    },
    setFilterQuery: function (query: string) {
...
class EditorClientDemo {
  ...
  private documentListAdapter: any;
  private tabAdapter: any;
...
async function demonstrateEditorModule() {
  ...
  try {
    ...
    documentService.on('documentAdded', (data: any) => {
...
if (typeof window === 'undefined') {
  ...
  (window as any).demonstrateEditorModule = demonstrateEditorModule;
}

export { demonstrateEditorModule, EditorClientDemo };
