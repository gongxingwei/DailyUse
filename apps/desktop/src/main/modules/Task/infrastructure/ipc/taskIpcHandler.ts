...
export class TaskIpcHandler {
  ...
  private registerMetaTemplateHandlers(): void {
    ...
    // 根据分类获取元模板
    ipcMain.handle(
      'task:meta-templates:get-by-category',
      withAuth(async (_event, [category], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
  ...
  private registerTaskTemplateHandlers(): void {
    ...
    // 根据关键结果获取任务模板
    ipcMain.handle(
      'task:templates:get-by-key-result',
      withAuth(async (_event, [goalUuid, keyResultId], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
        return await this.taskService.getTaskTemplateForKeyResult(
          auth.accountUuid,
          goalUuid,
          keyResultId,
        );
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [dto], auth) => {
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [uuid], auth) => {
        ...
        try {
          ...
          return {
            success: result,
            message: result ? 'Template activated successfully' : 'Failed to activate template',
          };
        } catch (error) {
          return {
            success: false,
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [uuid], auth) => {
        ...
        try {
          ...
          return {
            success: result,
            message: result ? 'Template paused successfully' : 'Failed to pause template',
          };
        } catch (error) {
          return {
            success: false,
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [uuid], auth) => {
        ...
        try {
          ...
          return {
            success: result,
            message: result ? 'Template resumed successfully' : 'Failed to resume template',
          };
        } catch (error) {
          return {
            success: false,
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [uuid], auth) => {
        ...
        try {
          ...
          return {
            success: result,
            message: result ? 'Template archived successfully' : 'Failed to archive template',
          };
        } catch (error) {
          return {
            success: false,
  ...
  private registerTaskInstanceHandlers(): void {
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [uuid, newScheduledTime, newEndTime], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
        try {
          // TODO: 实现重新安排任务时间的逻辑
          console.log(
            `Rescheduling task ${uuid} to ${newScheduledTime}`,
    ...
    // 提醒相关操作
    ipcMain.handle(
      'task:instances:trigger-reminder',
      withAuth(async (_event, [instanceId, alertId], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
        try {
    ...
    ipcMain.handle(
      ...
      withAuth(async (_event, [instanceId, alertId, snoozeUntil, reason], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
        try {
          // TODO: 实现推迟提醒的逻辑
          console.log(
            `Snoozing reminder ${alertId} for task ${instanceId} until ${snoozeUntil}`,
            reason ? `(${reason})` : '',
          );
    ...
    ipcMain.handle(
      'task:instances:dismiss-reminder',
      withAuth(async (_event, [instanceId, alertId], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
    ...
    ipcMain.handle(
      'task:instances:disable-reminders',
      withAuth(async (_event, [instanceId], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
        try {
    ...
    ipcMain.handle(
      'task:instances:enable-reminders',
      withAuth(async (_event, [instanceId], auth) => {
        if (!auth.accountUuid) {
          return { success: false, message: '未登录或登录已过期，请重新登录' };
        }
