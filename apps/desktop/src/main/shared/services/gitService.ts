import { GiApiResponse, GitLogResponse } from '@renderer/shared/types/git';
...
class GitService {
  ...
  async getStatus(): Promise<GiApiResponse> {
    if (!this.git) {
      return {
        success: false,
  ...
  async initialize(workingDirectory: string): Promise<GiApiResponse> {
    try {
  ...
  async unstageAll(): Promise<GiApiResponse> {
    if (!this.git) {
