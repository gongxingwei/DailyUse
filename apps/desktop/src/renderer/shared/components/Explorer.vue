...
const deleteFolder = async () => {
  if (!selectedNode.value) return;
  try {
    await fileSystem.delete(selectedNode.value.key, true);
    await refreshFolder();
  } catch (error) {
...
const deleteFile = async () => {
  if (!selectedNode.value) return;
  try {
    await fileSystem.delete(selectedNode.value.key, false);
    await refreshFolder();
  } catch (error) {
    console.error('Delete error:', error);
  }
...
async function refreshFolder() {
  if (folderData.value?.folderPath) {
    ...
    if (response.success && response.data) {
      folderData.value = response.data as FolderData;
    }
  }
}
...
watch(
  ...
  async (newPath) => {
    if (newPath) {
      try {
        await fileSystem.refreshFolder(newPath);
      } catch (error) {
...
.action-button:hover {
  background-color: var(--hover-color, rgba(102, 98, 98, 0.1));
}

.action-button:active {
  background-color: var(--active-color, rgba(129, 124, 124, 0.2));
}
