...
export class NotificationIpc {
  public static showNotification(options: NotificationWindowOptions): Promise<ApiResponse<void>> {
    return window.shared.ipcRenderer.invoke('show-notification', serializeForIpc(options));
  }

  public static closeNotification(uuid: string): Promise<ApiResponse<void>> {
