<template>
  <div class="source-control">
    ...
    <div class="source-control-header">
      ...
      <div class="header-actions">
        <v-icon class="action-icon" title="Refresh" @click="store.refreshStatus"
          >mdi-refresh</v-icon
        >
      </div>
    </div>
    ...
    <div v-if="store.isGitRepo" class="changes-container">
      ...
      <div v-if="store.isGitRepo" class="commit-section">
        ...
        ></textarea>
        <v-btn block :disabled="!canCommit" @click="handleCommit"> Commit </v-btn>
      </div>
      <!-- Staged Changes -->
      <div class="changes-group">
        <div class="section-header" @click="toggleStaged">
          <div class="section-header-left">
            <v-icon size="small">{{
              isStagedExpanded ? 'mdi-chevron-down' : 'mdi-chevron-right'
            }}</v-icon>
      ...
      <div class="changes-group">
        <div class="section-header" @click="toggleChanges">
          <div class="section-header-left">
            <v-icon size="small">{{
              isChangesExpanded ? 'mdi-chevron-down' : 'mdi-chevron-right'
            }}</v-icon>
            <span>Changes</span>
          </div>
          <div class="section-header-right">
            <div class="section-header-actions">
              ...
              <button
                ...
                @click.stop="store.stageAllChanges"
              >
            ...
            <span class="section-header-count">
              {{
                store.changes.modified.length +
                store.changes.deleted.length +
        ...
        <div v-show="isChangesExpanded" class="section-content">
          <template
            v-for="file in [
              ...store.changes.modified,
              ...store.changes.deleted,
              ...store.changes.not_added,
            ]"
            :key="file"
          >
    ...
    <div class="graph-header" @click="toggleGraph">
      <div class="section-header">
        <div class="section-header-left">
          <v-icon size="small">{{
            isGraphExpanded ? 'mdi-chevron-down' : 'mdi-chevron-right'
          }}</v-icon>
          <span>Timeline</span>
        </div>
      </div>
    ...
    <div
      v-if="store.isGitRepo"
      class="git-graph-container"
      :class="{ 'git-graph-collapsed': !isGraphExpanded }"
    >
      <GitGraph />
    </div>
  </div>
  <ConfirmDialog
    ...
    cancelText="Cancel"
    confirmText="Discard"
    @confirm="store.discardAllChanges"
  />
</template>
