<template>
  <div id="task-template-management">
    ...
    <v-dialog v-model="showDeleteAllDialog" max-width="500">
      <v-card>
        ...
        <v-card-text>
          ...
          <p class="mb-2">
            您即将删除
            <strong>{{ taskStore.getAllTaskTemplates.length }}</strong> 个任务模板，包括：
          </p>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showDeleteAllDialog = false"> 取消 </v-btn>
          <v-btn color="error" variant="elevated" @click="confirmDeleteAll"> 确认删除所有 </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <TemplateSelectionDialog
      ...
      @cancel="cancelTemplateSelection"
    ...
    <v-snackbar
      ...
      location="top right"
      variant="elevated"
      :multi-line="snackbar.message.length > 50"
    >
      {{ snackbar.message }}
    </v-snackbar>
  </div>
...
const confirmDeleteAll = async () => {
  try {
    ...
    const { getTaskDomainApplicationService } = await import(
      '@renderer/modules/Task/application/services/taskDomainApplicationService'
    );
