...
test.describe('Shortcut Settings', () => {
  ...
  test('should clear shortcut', async ({ page }) => {
    ...
    // 点击清除按钮
    const clearButton = shortcutInput.locator('..').getByRole('button', { name: /清除|Clear|close/i }).first();
  ...
  test('should restore individual shortcut to default', async ({ page }) => {
    ...
    // 点击恢复默认按钮
    const restoreButton = shortcutInput.locator('..').getByRole('button', { name: /恢复默认|Restore|restore/i }).first();
