...
export class TaskPage {
  ...
  async createTask(taskData: {
    ...
    // Fill title
    await this.page.fill('input[name="title"], input[placeholder*="标题"], input[label*="标题"]', taskData.title);
    ...
    // Submit
    await this.page.click('button[type="submit"], button:has-text("确定"), button:has-text("保存")');
  ...
  async createDependency(
    ...
    console.log(`[TaskPage] Creating dependency: ${targetTaskIdentifier} -> ${sourceTaskIdentifier}`);
    ...
    const sourceTitle = typeof sourceTaskIdentifier === 'string' ? sourceTaskIdentifier : `Task ${sourceTaskIdentifier}`;
  ...
  async deleteDependency(sourceTask: string | number, targetTask: string | number) {
    ...
    await this.page.click(`[data-dependency="${sourceTask}->${targetTask}"] button:has-text("删除")`);
  ...
  async expectTaskStatus(taskIdentifier: string | number, status: string) {
    const card = this.taskCard(taskIdentifier);
    const statusChip = card.locator(`[data-status="${status}"], .status-chip:has-text("${status}")`);
