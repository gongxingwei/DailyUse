<template>
    <v-list-item :key="task.uuid" class="task-item"
        :class="{ 'border-bottom': showBorder, 'completed-task': isCompleted }">
        <template v-slot:prepend>
            <!-- 未完成任务：显示完成按钮 -->
            <v-btn v-if="!isCompleted" icon variant="text" color="success" @click="$emit('complete', task.uuid)"
                class="complete-btn">
                <v-icon>mdi-circle-outline</v-icon>
            </v-btn>
            <!-- 已完成任务：显示完成图标 -->
            <v-icon v-else color="success" class="complete-icon">
                mdi-check-circle
            </v-icon>
        </template>
        ...
        <div class="task-content-wrapper">
            ...
            <v-list-item-subtitle class="task-meta">
                <v-icon size="small" class="mr-1">{{ isCompleted ? 'mdi-check' : 'mdi-clock-outline' }}</v-icon>
            ...
            <div v-if="task.goalLinks?.length" class="key-results mt-2">
                <v-chip v-for="link in task.goalLinks" :key="link.keyResultId" size="small" color="primary"
        ...
        <template v-slot:append>
            <!-- 已完成任务：显示撤销按钮 -->
            <v-btn v-if="isCompleted" icon variant="text" size="small" @click="$emit('undo', task.uuid)"
                class="undo-btn">
