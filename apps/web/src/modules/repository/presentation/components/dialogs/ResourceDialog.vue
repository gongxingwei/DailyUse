<template>
    <v-dialog :model-value="visible" height="500" width="700" class="resource-dialog" persistent>
        <v-card :style="{ backgroundColor: 'rgb(var(--v-theme-surface))' }" class="px-2 pb-2 d-flex flex-column"
            ...
            <v-card-text :style="{ backgroundColor: 'rgba(var(--v-theme-surface-light), 0.3)' }"
                ...
                <v-window v-model="activeTab" class="h-100 w-90">
                    ...
                    <v-window-item :value="0">
                        <v-form @submit.prevent class="px-4 py-2">
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field v-model="resourceName" :rules="nameRules" label="资源名称"
                                        placeholder="输入资源名称" required />
                                </v-col>
                            </v-row>

                            <v-select v-model="resourceType" :items="resourceTypeOptions" item-title="text"
                                item-value="value" label="资源类型" required>
                                <template v-slot:prepend-inner>
                                    <v-icon>{{ getResourceTypeIcon(resourceType) }}</v-icon>
                                </template>
                            </v-select>

                            <v-textarea v-model="resourceDescription" label="资源描述" rows="3"
                                placeholder="描述这个资源的内容和用途" />

                            <v-row>
                                <v-col cols="6">
                                    <v-text-field v-model="resourceFormat" label="文件格式" placeholder="如: PDF, DOCX, MP4">
                                        <template v-slot:prepend-inner>
                                ...
                                <v-col cols="6">
                                    <v-text-field v-model="resourceSizeDisplay" label="文件大小"
                                        placeholder="如: 10MB, 500KB" @update:model-value="parseFileSize">
                                        <template v-slot:prepend-inner>
                    ...
                    <v-window-item :value="1">
                        <div class="tags-section">
                            <div class="mb-6">
                                ...
                                <v-combobox v-model="resourceTags" label="添加标签" multiple chips clearable hide-details
                                    placeholder="输入标签并按回车添加...">
                                    <template v-slot:chip="{ props, item }">
                                        <v-chip v-bind="props" closable size="small" :color="getTagColor(item.raw)"
                                            variant="tonal">
                                            {{ item.raw }}
                                        </v-chip>
                                    </template>
                            ...
                            <div class="mb-6">
                                ...
                                <v-row>
                                    <v-col cols="6">
                                        <v-switch v-model="resourceSettings.isPublic" label="公开访问" color="primary"
                                            hide-details />
                                    </v-col>
                                    <v-col cols="6">
                                        <v-switch v-model="resourceSettings.allowDownload" label="允许下载" color="primary"
                                            hide-details />
                                    </v-col>
                                </v-row>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <v-rating v-model="resourcePriority" color="orange" size="large" :length="5" hover
                                    half-increments />
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-card variant="outlined" class="mb-4">
                                        ...
                                        <v-card-text>
                                            <v-text-field v-model="resourceViewUrl" label="查看链接"
                                                placeholder="用于在线查看的URL" density="comfortable" />

                                            <v-text-field v-model="resourceDownloadUrl" label="下载链接"
                                                placeholder="用于下载的URL" density="comfortable" />
                                        </v-card-text>
                                    </v-card>
                                </v-col>
                            </v-row>
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-card variant="outlined">
                                        ...
                                        <v-card-text>
                                            <v-textarea v-model="resourceNote" label="备注" placeholder="其他说明信息或使用注意事项"
                                                rows="4" density="comfortable" />
                                        </v-card-text>
                                    </v-card>
                                </v-col>
                            </v-row>
...
const getResourceTypeIcon = (type: RepositoryContracts.ResourceType) => {
    switch (type) {
        case RepositoryContracts.ResourceType.MARKDOWN: return 'mdi-file-document'
        case RepositoryContracts.ResourceType.PDF: return 'mdi-file-pdf-box'
        case RepositoryContracts.ResourceType.IMAGE: return 'mdi-image'
        case RepositoryContracts.ResourceType.VIDEO: return 'mdi-video'
        case RepositoryContracts.ResourceType.AUDIO: return 'mdi-music'
        case RepositoryContracts.ResourceType.CODE: return 'mdi-code-tags'
        case RepositoryContracts.ResourceType.LINK: return 'mdi-link'
