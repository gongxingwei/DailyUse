                ...
                <v-window v-model="activeTab" class="h-100 w-90">
                    ...
                    <v-window-item :value="0">
                        <v-form @submit.prevent class="px-4 py-2">
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field v-model="repositoryName" :rules="nameRules" label="仓库名称"
                                        placeholder="输入仓库名称" required />
                                </v-col>
                            </v-row>

                            <v-select v-model="repositoryType" :items="repositoryTypeOptions" item-title="text"
                                item-value="value" label="仓库类型" required>
                                <template v-slot:prepend-inner>
                                    <v-icon>mdi-folder-outline</v-icon>
                                </template>
                            </v-select>

                            <v-textarea v-model="repositoryDescription" label="仓库描述" rows="3"
                                placeholder="描述这个仓库的用途和内容" />

                            <v-text-field v-model="repositoryLocation" label="存储位置" placeholder="文件系统路径或远程URL">
                                <template v-slot:prepend-inner>
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-select v-model="repositoryStatus" :items="statusOptions" item-title="text"
                                        item-value="value" label="状态">
                                        <template v-slot:prepend-inner>
                    ...
                    <v-window-item :value="1">
                        <div class="related-goals-section">
                            <div v-if="repositoryData.relatedGoals && repositoryData.relatedGoals.length > 0"
                                class="mb-4">
                                <h4 class="text-h6 mb-3">已关联的目标 ({{ repositoryData.relatedGoals.length }})</h4>
                                <v-list>
                                    <v-list-item v-for="(goalUuid, index) in repositoryData.relatedGoals"
                                        :key="`goal-${index}`" class="mb-2">
                            ...
                            <v-select v-model="selectedGoalToAdd" :items="availableGoals" item-title="name"
                                item-value="uuid" label="选择要关联的目标" clearable @update:model-value="addRelatedGoal">
                                <template v-slot:prepend-inner>
                            ...
                            <v-row>
                                <v-col cols="12" md="6">
                                    <v-card variant="outlined" class="h-100">
                                        ...
                                        <v-card-text>
                                            <v-switch v-model="repositorySettings.autoSync" label="自动同步" color="primary"
                                                hide-details class="mb-4" />

                                            <v-switch v-model="repositorySettings.enableVersionControl" label="启用版本控制"
                                                color="primary" hide-details class="mb-4" />

                                            <v-switch v-model="repositorySettings.enableGit" label="启用 Git"
                                ...
                                <v-col cols="12" md="6">
                                    <v-card variant="outlined" class="h-100">
                                        ...
                                        <v-card-text>
                                            <v-combobox v-model="repositoryTags" label="标签" multiple chips clearable
                                                hide-details placeholder="添加标签...">
                                                <template v-slot:chip="{ props, item }">
                                                    <v-chip v-bind="props" closable size="small" color="primary"
                                                        variant="tonal">
                                                        {{ item.raw }}
                                                    </v-chip>
                                                </template>
...
const removeRelatedGoal = (goalUuid: string) => {
    repositoryData.value.relatedGoals = repositoryData.value.relatedGoals.filter(id => id !== goalUuid)
}
