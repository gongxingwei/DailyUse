    ...
    <div :class="isSidebarVisible ? 'editor-layout' : 'editor-layout-sidebar-hidden'">
        ...
        <div class="editor-groups" :class="{ 'sidebar-hidden': !isSidebarVisible }">
            <template v-for="(group, index) in editorGroupStore.editorGroups" :key="`group-${group.uuid}`">
                <EditorGroup :group-id="group.uuid"
                    :class="{ 'active': group.uuid === editorGroupStore.activeGroupId }" />
                <!-- 在非最后一个编辑器组后添加 ResizeHandle -->
                <ResizeHandle v-if="index < editorGroupStore.editorGroups.length - 1" :key="`resize-${group.uuid}`"
                    type="editor-editor" :group-id="group.uuid" class="editor-resize-handle" />
            </template>
        </div>
        <StatusBar />
    </div>
...
.resize-handle-siderbar.sidebar-hidden {
    grid-column: 2;
    /* Move to column 2 when sidebar is hidden */
}
