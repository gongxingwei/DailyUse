<template>
    <div class="goal-review-card-demo">
        ...
        <div class="demo-controls mb-4">
            ...
            <v-btn color="secondary" @click="selectRandomGoal" class="ml-2" :disabled="goals.length === 0">
                随机选择目标
            </v-btn>
            <v-btn color="info" @click="refreshGoals" class="ml-2">
                刷新目标列表
            </v-btn>
        </div>
        ...
        <v-card class="mb-4" variant="outlined">
            ...
            <v-card-text>
                <v-select v-model="selectedGoal" :items="goals" item-title="name" item-value="uuid" return-object
                    label="选择一个目标查看复盘" variant="outlined" density="comfortable">
        ...
        <v-card v-if="selectedGoal" class="mb-4" variant="outlined">
            ...
            <v-card-text>
                ...
                <div class="d-flex gap-4">
                    <v-chip color="primary" size="small" variant="tonal">
                        ...
                        {{ format(selectedGoal.startTime, 'yyyy-MM-dd') }} - {{ format(selectedGoal.endTime,
                        'yyyy-MM-dd') }}
                    </v-chip>
            ...
            <v-card-actions>
                <v-btn color="primary" variant="elevated" prepend-icon="mdi-book-edit" @click="openReviewDialog">
                    查看复盘记录
                </v-btn>
            </v-card-actions>
        </v-card>
