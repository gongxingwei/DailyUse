<template>
    <div class="goal-card-demo">
        ...
        <div class="demo-controls mb-4">
            ...
            <v-btn color="secondary" @click="refreshGoals" class="ml-2">
                刷新目标列表
            </v-btn>
        </div>

        <div class="goals-grid">
            <GoalCard v-for="goal in goals" :key="goal.uuid" :goal="goal" ref="goalCardRefs" @click="selectGoal(goal)"
...
const selectedGoalRef = computed(() => {
    ...
    return goalCardRefs.value.find((cardRef, index) =>
        goals.value[index]?.uuid === selectedGoal.value?.uuid
    )
})
...
.goal-card.selected:hover {
    box-shadow: 0 0 0 2px rgb(var(--v-theme-primary)), 0 12px 40px rgba(0, 0, 0, 0.15);
}
</style>
