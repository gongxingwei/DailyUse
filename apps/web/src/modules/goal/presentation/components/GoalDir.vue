    ...
    <v-card class="goal-dir h-100 d-flex flex-column" elevation="0" variant="outlined">
        ...
        <v-card-title class="goal-dir-header d-flex align-center justify-space-between pa-4">
            ...
            <v-menu>
                <template v-slot:activator="{ props: menuProps }">
                    <v-btn v-bind="menuProps" icon="mdi-plus" size="small" variant="text" color="primary"
                        class="add-btn">
        ...
        <v-card-text class="goal-dir-list pa-0 flex-grow-1 overflow-y-auto">
            <v-list class="py-0" density="compact">
                <!-- 全部目标 -->
                <v-list-item :class="{ 'goal-dir-item--active': selectedDirUuid === 'all' }"
                    class="goal-dir-item mx-2 my-1" @click="selectDir('all')" rounded="lg">
                    <template v-slot:prepend>
                    ...
                    <template v-slot:append>
                        <v-chip :color="selectedDirUuid === 'all' ? 'primary' : 'surface-bright'"
                            :text-color="selectedDirUuid === 'all' ? 'on-primary' : 'on-surface-variant'" size="small"
                ...
                <!-- 动态目标分类 -->
                <v-list-item v-for="dir in goalDirs" :key="dir.uuid"
                    :class="{ 'goal-dir-item--active': selectedDirUuid === dir.uuid }" class="goal-dir-item mx-2 my-1"
                    @click="selectDir(dir.uuid)" rounded="lg">
                    <template v-slot:prepend>
                    ...
                    <template v-slot:append>
                        <div class="d-flex align-center">
                            <v-chip :color="selectedDirUuid === dir.uuid ? 'primary' : 'surface-bright'"
                            ...
                            <!-- 系统目录不显示编辑按钮 -->
                            <v-btn v-if="!dir.isSystemDir" icon="mdi-pencil" size="x-small" variant="text"
                                color="medium-emphasis" class="ml-1" @click.stop="$emit('edit-goal-dir', dir)">
                ...
                <!-- 已归档 -->
                <v-list-item v-if="archivedGoalsCount > 0"
                    :class="{ 'goal-dir-item--active': selectedDirUuid === 'archived' }" class="goal-dir-item mx-2 my-1"
                    @click="selectDir('archived')" rounded="lg">
                    <template v-slot:prepend>
                    ...
                    <template v-slot:append>
                        <v-chip :color="selectedDirUuid === 'archived' ? 'primary' : 'surface-bright'"
