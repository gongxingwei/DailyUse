            ...
            <v-card-text class="pa-4">
                    ...
                    <v-card-text>
                        <div class="d-flex justify-space-between align-start">
                            ...
                            <div class="text-right">
                                <v-switch v-model="groupEnabled" :color="group.color || 'primary'" label="启用组"
                                    hide-details @change="toggleGroupEnabled" />
                            </div>
                        </div>
                    </v-card-text>
                </v-card>
                ...
                <div class="templates-desktop">
                    ...
                    <div class="desktop-area" @contextmenu.prevent="showDesktopContextMenu">
                        <div v-for="template in templates" :key="template.uuid" class="template-item"
                            @click="openTemplateCard(template)"
                            @contextmenu.prevent="showTemplateContextMenu($event, template)">
                            <v-card :color="template.color || 'primary'"
                                :class="{ 'template-disabled': !template.enabled }" elevation="2" hover>
...
// 监听组启用状态
watch(() => group.value?.enabled, (newVal) => {
