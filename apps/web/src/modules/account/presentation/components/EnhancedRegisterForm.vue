...
<template>
  <v-form ref="formRef" @submit.prevent="handleSubmit">
    <v-container>
      <!-- 错误提示 -->
      <v-alert
        v-if="formState.errors.value.length > 0"
        type="error"
        variant="tonal"
        class="mb-4"
        closable
      ...
      <v-row class="mb-4">
        <v-col cols="12">
          <v-chip-group v-model="validationMode" mandatory>
            ...
            <v-chip value="universal" color="success" variant="outlined"> 通用校验系统 </v-chip>
          </v-chip-group>
        </v-col>
      </v-row>
      ...
      <v-row>
        <v-col cols="12">
          ...
          <div v-if="validationMode === 'universal' && formState.fields.username" class="mt-1">
            <v-chip
              size="small"
              :color="formState.fields.username.valid.value ? 'success' : 'error'"
              variant="outlined"
            >
              {{ formState.fields.username.valid.value ? '有效' : '无效' }}
            </v-chip>
            <v-chip
              v-if="formState.fields.username.dirty.value"
              size="small"
              color="info"
              variant="outlined"
              class="ml-1"
            ...
            <v-chip
              v-if="formState.fields.username.touched.value"
              size="small"
              color="warning"
              variant="outlined"
              class="ml-1"
      ...
      <v-row>
        <v-col cols="12">
          ...
          <v-text-field
            ...
            label="邮箱 (通用校验)"
            :error-messages="
              formState.fields.email?.error.value ? [formState.fields.email.error.value] : []
            "
            :loading="formState.fields.email?.validating.value"
            prepend-inner-icon="mdi-email"
            clearable
            required
        ...
        <v-col cols="12">
          ...
          <div v-if="validationMode === 'universal' && passwordValue" class="mt-2">
            ...
            <div v-if="formState.fields.password?.warnings.value.length > 0" class="mt-2">
              <v-alert
                v-for="warning in formState.fields.password.warnings.value"
                :key="warning"
                type="warning"
                variant="tonal"
                density="compact"
      ...
      <v-row>
        <v-col cols="12">
          ...
          <v-text-field
            ...
            :type="showConfirmPassword ? 'text' : 'password'"
            :error-messages="
              formState.fields.confirmPassword?.error.value
                ? [formState.fields.confirmPassword.error.value]
                : []
            "
          ...
          <v-card variant="outlined" class="pa-3">
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="6" sm="3">
                  <v-chip
                    :color="formState.touched.value ? 'info' : 'default'"
                    variant="outlined"
                    block
                  >
                ...
                <v-col cols="6" sm="3">
                  <v-chip
                    :color="formState.dirty.value ? 'warning' : 'default'"
                    variant="outlined"
                    block
                  >
                ...
                <v-col cols="6" sm="3">
                  <v-chip
                    :color="formState.validating.value ? 'purple' : 'default'"
                    variant="outlined"
                    block
                  >
...
import {
  ...
  passwordRules,
  emailRules,
} from '../../../../shared/utils/validations/rules';
...
const passwordStrength = computed(() => {
  const password =
    validationMode.value === 'legacy' ? registrationData.password : passwordValue.value || '';
...
const handleSubmit = async () => {
  ...
  try {
    ...
    showMessage(
      `注册成功! 使用 ${validationMode.value === 'legacy' ? '传统' : '通用'} 校验`,
      'success',
    );
