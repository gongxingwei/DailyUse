        ...
        <v-card-text class="pt-0">
            ...
            <div v-else>
                <v-list class="pa-0" density="compact">
                    ...
                    <v-list-item v-for="user in localUsers" :key="`${user.username}-${user.accountType}`" class="mb-2"
                        ...
                        <v-list-item-subtitle class="text-caption">
                            <v-chip size="x-small" color="secondary" variant="flat" class="mr-1">
                                本地账户
                            </v-chip>
                        ...
                        <template v-slot:append>
                            ...
                            <!-- 快速登录按钮 -->
                            <v-btn size="small" variant="flat" color="secondary" :loading="loading === user.username"
                ...
                <div class="d-flex justify-space-between align-center">
                    <v-btn size="small" variant="text" color="secondary" prepend-icon="mdi-refresh"
                        @click="refreshLocalUsers" :loading="refreshing">
                        刷新列表
                    </v-btn>

                    <v-btn size="small" variant="text" color="error" prepend-icon="mdi-delete-sweep"
                        @click="handleClearAllLocalAccounts" :disabled="!!loading || localUsers.length === 0">
                        清除全部
                    </v-btn>
                </div>
            </div>
...
import { loginSessionService } from '../../../Account/domain/services/loginSessionService';

// 使用 useUserAuth 的状态和方法
const { rememberedUsers, snackbar, loadRememberedUsers, handleLocalQuickLogin } = useUserAuth();
...
const localUsers = computed(() => {
    return rememberedUsers.value.filter(user => user.accountType === 'local');
});
...
const handleQuickLogin = async (username: string) => {
    ...
    try {
        // 调用 useUserAuth 中的本地快速登录方法
        await handleLocalQuickLogin(username);

        // 刷新用户列表（更新最后登录时间）
        await loadRememberedUsers();
    } catch (error) {
        console.error('本地快速登录失败:', error);
        snackbar.show = true;
 ...
 * 清除所有本地账户凭证
 */
const handleClearAllLocalAccounts = () => {
    confirmDialog.value = {
        show: true,
        title: '清除全部本地账户',
        message: `确定要清除所有 ${localUsers.value.length} 个本地账户的登录凭证吗？此操作不可撤销。`,
        action: async () => {
            confirmDialog.value.loading = true;
            try {
                const localSessions = localUsers.value.map(user => ({
                    username: user.username,
                    accountType: user.accountType,
                }));

                const result = await loginSessionService.batchRemoveSessions(localSessions);
                if (result.success) {
                    await loadRememberedUsers();
                    snackbar.show = true;
                    snackbar.message = '所有本地账户凭证已清除';
                    snackbar.color = 'success';
                } else {
                    throw new Error(result.message || '清除失败');
                }
            } catch (error) {
                console.error('清除所有本地账户失败:', error);
                snackbar.show = true;
                snackbar.message = '清除本地账户凭证失败，请稍后重试';
                snackbar.color = 'error';
            } finally {
                confirmDialog.value.loading = false;
                confirmDialog.value.show = false;
