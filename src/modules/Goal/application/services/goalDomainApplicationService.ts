...
export class GoalDomainApplicationService {
   ...
   * 创建目标
   */
  async createGoal(goal: Goal): Promise<TResponse<Goal>> {
    try {
   ...
   * 更新目标
   */
  async updateGoal(goal: IGoal): Promise<TResponse<{ goal: IGoal }>> {
    try {
  ...
  async addReviewToGoal(goalReview: GoalReview): Promise<TResponse<Goal>> {
    try {
  ...
  async updateReviewInGoal(goalReview: GoalReview): Promise<TResponse<Goal>> {
    try {
      // 调用主进程的聚合根方法
  ...
  async removeReviewFromGoal(
    ...
    try {
      ...
      // 调用主进程的聚合根方法
      const response = await goalIpcClient.removeReviewFromGoal(
        goalReview
      );
  ...
  async createGoalDir(
    goalDir: GoalDir
  ): Promise<TResponse<{ goalDir: GoalDir }>> {
    try {
  ...
  async getAllGoalDirs(): Promise<GoalDir[]> {
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log(
          `✅ [目标应用服务] 获取目标目录成功，数量: ${response.data.length}`
        );
        return goalDirs;
      }
  ...
  async updateGoalDir(
    goalDir: GoalDir
  ): Promise<TResponse<{ goalDir: IGoalDir }>> {
    try {
 ...
 * 获取目标领域应用服务的默认实例
 */
let _goalDomainApplicationServiceInstance: GoalDomainApplicationService | null =
  null;
