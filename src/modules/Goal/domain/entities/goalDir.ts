...
import type { IGoalDir, IGoalDirCreateDTO } from "../types/goal";
...
export class GoalDir extends AggregateRoot implements IGoalDir {
   ...
   * 从创建数据传输对象创建目录
   */
  static fromCreateDTO(id: string, data: IGoalDirCreateDTO): GoalDir {
    return new GoalDir(
      id,
      data.name,
      data.icon,
   ...
   * 验证目录数据
   */
  static validate(data: IGoalDirCreateDTO): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
