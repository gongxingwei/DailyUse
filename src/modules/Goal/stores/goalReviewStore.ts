...
export const useGoalReviewStore = defineStore("goalReview", {
  ...
  actions: {
    async addGoalReview(review: Review) {
      this.reviews.push(review);

      // 自动保存
      const saveSuccess = await this.saveGoalReviews();
      if (!saveSuccess) {
        console.error("复盘记录添加后保存失败");
      }
    },
    ...
    initTempReview(goalId: string): Review {
      ...
      // 获取关键结果进度
      const keyResultProgress: KeyResultProgress[] = goal.keyResults.map(
        (kr) => ({
          id: kr.id,
          name: kr.name,
    ...
    // 自动保存方法
    async saveGoalReviews(): Promise<boolean> {
      const autoSave = getAutoSave();
      return autoSave.debounceSave("goalReviews", this.reviews);
    },

    async saveGoalReviewsImmediately(): Promise<boolean> {
      const autoSave = getAutoSave();
      return autoSave.saveImmediately("goalReviews", this.reviews);
    },

    // 检查保存状态
    isSavingGoalReviews(): boolean {
      const autoSave = getAutoSave();
      return autoSave.isSaving("goalReviews");
    },
  },
});
