...
import type { ReminderRule, ReminderAlert } from '../types/timeStructure';
...
export function useReminderValidation() {
   ...
   * 前端快速验证 - 仅用于UI实时反馈
   */
  const validateReminders = (reminderConfig: ReminderRule): boolean => {
    errors.value = [];
    warnings.value = [];
   ...
   * 单个提醒项的快速验证
   */
  const validateSingleAlert = (alert: ReminderAlert): boolean => {
    if (!alert.type) {
   ...
   * 检查提醒时间冲突 (UI层面的简单检查)
   */
  const checkTimeConflicts = (alerts: ReminderAlert[]): boolean => {
