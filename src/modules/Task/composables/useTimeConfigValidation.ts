...
import type { TaskTimeConfig } from '../types/timeStructure';
...
export function useTimeConfigValidation() {
   ...
   * 快速检查时间冲突 (简单的UI层检查)
   */
  const checkBasicTimeConflict = (timeConfig: TaskTimeConfig, otherTimes: TaskTimeConfig[] = []): boolean => {
   ...
   * 验证重复设置的合理性
   */
  const validateRecurrenceSettings = (timeConfig: TaskTimeConfig): boolean => {
