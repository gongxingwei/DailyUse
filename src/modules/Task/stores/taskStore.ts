...
export const useTaskStore = defineStore("task", {
  ...
  getters: {
    ...
    getTaskStatsForGoal: (state) => (goalId: string) => {
      ...
      const overallStats = {
        ...
        missedTasks: tasks.filter(
          (t) =>
            t.status !== "completed" &&
            t.scheduledTime.timestamp < now.timestamp
        ).length,
      };
    ...
    getTaskCompletionTimeline:
      (state) => (goalId: string, startDate: string, endDate: string) => {
        ...
        const start = TimeUtils.fromISOString(
        ...
        while (currentDate.timestamp <= end.timestamp) {
          const dateStr = `${currentDate.date.year}-${currentDate.date.month
            .toString()
            .padStart(2, "0")}-${currentDate.date.day
            .toString()
            .padStart(2, "0")}`;
          timeline[dateStr] = {
            total: 0,
  ...
  actions: {
      ...
      if (taskIndex !== -1) {
        ...
        if (task.keyResultLinks?.length) {
          const event: TaskCompletedEvent = {
            ...
            data: {
              taskId,
              keyResultLinks: task.keyResultLinks,
    ...
    async initializeSchedules() {
      ...
      await reminderService.reinitializeAllReminders(
        this.taskTemplates,
        this.taskInstances
      );
    },
