...
import { useRecurrenceValidation } from '@/modules/Task/composables/useRecurrenceValidation';
...
const handleRecurrenceChange = (type: string | null) => {
  if (!type) {
    localData.value = {
      ...localData.value,
      timeConfig: {
        ...localData.value.timeConfig,
        recurrence: {
          ...localData.value.timeConfig.recurrence,
          endCondition: {
            type: 'never'
          }
        }
...
const updateWeekdays = (weekdays: number[]) => {
  if (!localData.value.timeConfig.recurrence.config) {
    localData.value.timeConfig.recurrence.config = {};
  }
  localData.value.timeConfig.recurrence.config.weekdays = [...weekdays];
};
  ...
  localData.value = {
    ...
    timeConfig: {
      ...
      recurrence: {
        ...localData.value.timeConfig.recurrence,
        endCondition: {
          ...localData.value.timeConfig.recurrence.endCondition,
          endDate
        }
      }
    }
