...
export class ConditionalValidator implements ITemplateValidator {
  constructor(
    private condition: (template: TaskTemplate) => boolean,
...
export class ValidationRuleBuilder {
  private rules: Array<(template: TaskTemplate) => ValidationResult> = [];

  /**
   * 添加字段验证规则
   */
  field<T>(
    fieldGetter: (template: TaskTemplate) => T,
   ...
   */
  custom(
    rule: (template: TaskTemplate) => ValidationResult
  ): ValidationRuleBuilder {
   ...
   */
  when(
    condition: (template: TaskTemplate) => boolean,
    rule: (template: TaskTemplate) => ValidationResult
  ): ValidationRuleBuilder {
    this.rules.push((template) => {
  ...
  build(): ITemplateValidator {
    ...
    return {
      validate: (template: TaskTemplate) => {
