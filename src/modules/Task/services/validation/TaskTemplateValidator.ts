...
import { BasicInfoValidator } from './validators/BasicInfoValidator';
import { TimeConfigValidator } from './validators/TimeConfigValidator';
import { RecurrenceValidator } from './validators/RecurrenceValidator';
import { ReminderValidator } from './validators/ReminderValidator';
import { SchedulingPolicyValidator } from './validators/SchedulingPolicyValidator';
...
export class TaskTemplateValidator {
   ...
   * 快速验证（只检查关键字段）
   */
  static quickValidate(template: TaskTemplate): ValidationResult {
   ...
   * 验证用于创建的模板
   */
  static validateForCreate(template: TaskTemplate): ValidationResult {
    const context: ValidationContext = {
      mode: 'create',
      strict: false
    };
   ...
   * 严格验证（所有验证器都必须通过）
   */
  static strictValidate(template: TaskTemplate): ValidationResult {
    const context: ValidationContext = {
      mode: 'create',
      strict: true
    };
