...
export class MetadataValidator implements ITemplateValidator {
   ...
   * 验证元数据组合的合理性
   */
  private validateMetadataCombination(metadata: any, template: TaskTemplate): ValidationResult {
    const warnings: string[] = [];
    ...
    if (categoryLower === 'work' && !tagLower.some((tag: string) => 
      ['工作', 'work', '项目', 'project', '会议', 'meeting'].includes(tag)
    )) {
      warnings.push('工作类任务建议添加相关标签');
    }

    if (categoryLower === 'health' && !tagLower.some((tag: string) => 
      ['健康', 'health', '运动', 'exercise', '锻炼'].includes(tag)
    )) {
      warnings.push('健康类任务建议添加相关标签');
    }

    // 难度与时长的匹配性
    if (metadata.difficulty <= 2 && metadata.estimatedDuration && metadata.estimatedDuration > 3 * 60) {
