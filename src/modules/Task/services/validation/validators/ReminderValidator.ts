...
import type { ReminderRule } from '../../../types/timeStructure';
...
export class ReminderValidator implements ITemplateValidator {
  validate(template: TaskTemplate): ValidationResult {
    const reminder = template.timeConfig?.reminder;
   ...
   * 验证未启用的提醒配置
   */
  private validateDisabledReminder(reminder: ReminderRule): ValidationResult {
   ...
   * 验证业务逻辑
   */
  private validateBusinessLogic(reminder: ReminderRule, template: TaskTemplate): ValidationResult {
    const warnings: string[] = [];
    const errors: string[] = [];
