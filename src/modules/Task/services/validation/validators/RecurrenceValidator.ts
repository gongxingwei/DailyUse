...
import type { RecurrenceRule } from '../../../types/timeStructure';
...
export class RecurrenceValidator implements ITemplateValidator {
  validate(template: TaskTemplate): ValidationResult {
    ...
    // 验证结束条件
    results.push(this.validateEndCondition(recurrence.endCondition, recurrence.type));

    // 验证重复配置
    if (recurrence.config) {
      results.push(this.validateRecurrenceConfig(recurrence.config, recurrence.type));
    }
   ...
   * 验证重复间隔
   */
  private validateInterval(interval: number | undefined, type: string): ValidationResult {
    if (type === 'none') {
    ...
    // 基本数值验证
    const numberResult = ValidationUtils.validateNumberRange(interval, '重复间隔', {
      min: 1,
   ...
   * 验证结束条件
   */
  private validateEndCondition(endCondition: any, recurrenceType: string): ValidationResult {
    if (!endCondition) {
    ...
    switch (endCondition.type) {
      ...
      case 'never':
        ...
        if (recurrenceType !== 'none') {
          results.push(ValidationUtils.success(['任务设置为永不结束，请确认这是预期行为']));
        }
        break;
    }
   ...
   * 验证结束次数
   */
  private validateEndCount(count: number | undefined, recurrenceType: string): ValidationResult {
    if (count === undefined) {
      return ValidationUtils.failure(['结束次数不能为空']);
    }

    const numberResult = ValidationUtils.validateNumberRange(count, '结束次数', {
      min: 1,
      max: 10000,
      required: true,
      integer: true
   ...
   * 验证重复配置
   */
  private validateRecurrenceConfig(config: any, type: string): ValidationResult {
    const results: ValidationResult[] = [];
  ...
  private validateMonthDays(monthDays: number[]): ValidationResult {
    const arrayResult = ValidationUtils.validateArray(monthDays, '月份日期配置', {
      required: true,
      minLength: 1,
      maxLength: 31
    });
  ...
  private validateMonthWeekdays(monthWeekdays: any[]): ValidationResult {
    const arrayResult = ValidationUtils.validateArray(monthWeekdays, '月份星期配置', {
      required: true,
      minLength: 1,
      maxLength: 10
    });
    ...
    for (const item of monthWeekdays) {
      if (!item.week || !Number.isInteger(item.week) || 
          (item.week < -1 || item.week === 0 || item.week > 5)) {
   ...
   * 验证业务逻辑
   */
  private validateBusinessLogic(recurrence: RecurrenceRule, template: TaskTemplate): ValidationResult {
    const warnings: string[] = [];
    const errors: string[] = [];
    ...
    if (recurrence.type === 'daily' && recurrence.interval === 1) {
      if (template.timeConfig.type === 'timeRange') {
        const duration = template.timeConfig.baseTime.end?.timestamp! - 
    ...
    // 检查习惯养成类任务
    if (recurrence.type === 'daily' && recurrence.endCondition.type === 'count') {
