...
import { remoteUserService } from "../../domain/services/remoteUserService";
import { remoteAuthService } from "../../domain/services/remoteAuthService";
import { loginSessionService } from "../../domain/services/loginSessionService";
// types
import type { AccountRegistrationRequest } from "../../domain/types/account";
import type { SessionUser } from "../../domain/services/loginSessionService";
...
export function useUserAuth(emit?: {
  ...
  const registerForm = reactive<AccountRegistrationRequest>({
    username: "",
    firstName: "",
    lastName: "",
    sex: "",
  ...
  //     // 调用登录服务
  //     const response = await accountApplicationService.login(loginForm);
  ...
  async function handleLocalRegister() {
    try {
      ...
      const response = await accountApplicationService.register(registerForm);
  ...
  const handleLocalQuickLogin = async (username: string) => {
    try {
      ...
      if (sessionResult.success) {
        // 登录成功，保存用户信息
        accountStore.setUser(sessionResult.data);
        // 重新加载记住的用户列表
        await loadRememberedUsers();
        router.push("/");
      } else {
  ...
  async function handleRemoteLogin() {
    try {
      ...
      if (response.success) {
        ...
        accountStore.setUser(response.data.userWithoutPassword);
        await loadRememberedUsers();
        router.push("/");
      }
    } catch (error) {
