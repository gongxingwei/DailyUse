 ...
 * Vue 表单返回类型
 */
export interface VueFormReturn {
  state: VueFormState;
 ...
 * Vue 表单校验组合函数
 */
export function useFormValidation(options: UseFormValidationOptions): VueFormReturn {
  const {
    config,
    initialValues = {},
  ...
  const state: VueFormState = {
    values,
    fields,
    valid: computed(() => Object.values(fields).every((field) => field.valid.value)),
    validating: computed(() => Object.values(fields).some((field) => field.validating.value)),
    touched: computed(() => Object.values(fields).some((field) => field.touched.value)),
    dirty: computed(() => Object.values(fields).some((field) => field.dirty.value)),
    errors: computed(() => Object.values(fields).flatMap((field) => field.errors.value)),
    warnings: computed(() => Object.values(fields).flatMap((field) => field.warnings.value)),
  ...
  async function validateForm(
    ...
    try {
      ...
      globalWarnings.value = result.warnings;
   ...
   * 添加规则
   */
  function addRule(fieldName: string, rule: ValidationRule): void {
   ...
   * 移除规则
   */
  function removeRule(fieldName: string, ruleType: string): void {
    validator.removeRule(fieldName, ruleType);
  }
   ...
   * 更新字段状态
   */
  function updateFieldState(fieldName: string, result: FieldValidationResult): void {
    const fieldState = fields[fieldName];
    if (!fieldState) return;
