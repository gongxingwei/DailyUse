...
export class GoalStatistics extends AggregateRoot implements IGoalStatisticsServer {
  ...
  public static fromPersistenceDTO(dto: GoalStatisticsPersistenceDTO): GoalStatistics {
    return new GoalStatistics({
      ...
      totalGoals: dto.total_goals,
      activeGoals: dto.active_goals,
      completedGoals: dto.completed_goals,
      archivedGoals: dto.archived_goals,
      overdueGoals: dto.overdue_goals,
      totalKeyResults: dto.total_key_results,
      completedKeyResults: dto.completed_key_results,
      averageProgress: dto.average_progress,
      goalsByImportance: JSON.parse(dto.goals_by_importance) as Record<string, number>,
      goalsByUrgency: JSON.parse(dto.goals_by_urgency) as Record<string, number>,
      goalsByCategory: JSON.parse(dto.goals_by_category) as Record<string, number>,
      goalsByStatus: JSON.parse(dto.goals_by_status) as Record<string, number>,
      goalsCreatedThisWeek: dto.goals_created_this_week,
      goalsCompletedThisWeek: dto.goals_completed_this_week,
      goalsCreatedThisMonth: dto.goals_created_this_month,
      goalsCompletedThisMonth: dto.goals_completed_this_month,
      totalReviews: dto.total_reviews,
      averageRating: dto.average_rating ?? null,
      lastCalculatedAt: dto.last_calculated_at,
    });
  }
  ...
  public toPersistenceDTO(): GoalStatisticsPersistenceDTO {
    return {
      ...
      total_goals: this._totalGoals,
      active_goals: this._activeGoals,
      completed_goals: this._completedGoals,
      archived_goals: this._archivedGoals,
      overdue_goals: this._overdueGoals,
      total_key_results: this._totalKeyResults,
      completed_key_results: this._completedKeyResults,
      average_progress: this._averageProgress,
      goals_by_importance: JSON.stringify(this._goalsByImportance),
      goals_by_urgency: JSON.stringify(this._goalsByUrgency),
      goals_by_category: JSON.stringify(this._goalsByCategory),
      goals_by_status: JSON.stringify(this._goalsByStatus),
      goals_created_this_week: this._goalsCreatedThisWeek,
      goals_completed_this_week: this._goalsCompletedThisWeek,
      goals_created_this_month: this._goalsCreatedThisMonth,
      goals_completed_this_month: this._goalsCompletedThisMonth,
      total_reviews: this._totalReviews,
      average_rating: this._averageRating,
      last_calculated_at: this._lastCalculatedAt,
    };
  }
}
