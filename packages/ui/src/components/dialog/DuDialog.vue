<template>
    <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" :width="width"
        :max-width="maxWidth" :persistent="persistent" :no-click-animation="noClickAnimation">
        <v-card :class="cardClass">
            ...
            <v-card-title v-if="title || $slots.title" class="d-flex align-center">
                ...
                <v-btn v-if="showCloseButton" icon="mdi-close" variant="text" size="small"
            ...
            <v-card-actions v-else-if="showDefaultActions" class="justify-end">
                ...
                <v-btn v-if="showConfirmButton" :color="confirmColor" :variant="confirmVariant" @click="handleConfirm"
                    :loading="loading">
                    {{ confirmText }}
                </v-btn>
            </v-card-actions>
