<template>
    <v-form ref="formRef" @submit.prevent="handleSubmit" :loading="loading">
        <v-container>
            ...
            <v-row>
                <v-col cols="12">
                    <DuTextField v-model="formData.username" label="用户名或邮箱" :rules="loginUsernameRules"
                        prepend-inner-icon="mdi-account" autocomplete="username" autofocus required>
            ...
            <v-row>
                <v-col cols="12">
                    <DuTextField v-model="formData.password" label="密码" type="password" :rules="loginPasswordRules"
            ...
            <v-row>
                <v-col cols="12">
                    <v-checkbox v-model="formData.remember" label="记住我" color="primary" density="compact"
                        hide-details />
                </v-col>
            </v-row>
            ...
            <v-row class="mt-4">
                <v-col cols="12" class="text-center">
                    <v-btn color="primary" type="submit" :loading="loading" :disabled="!isFormValid" size="large" block>
            ...
            <v-row v-if="showSocialLogin" class="mt-4">
                <v-col cols="12">
                    ...
                    <div class="d-flex justify-center">
                        <v-btn v-for="provider in socialProviders" :key="provider.name" :color="provider.color"
                            variant="outlined" class="mx-1" @click="$emit('social-login', provider.name)">
