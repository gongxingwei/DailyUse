    ...
    <v-form ref="formRef" @submit.prevent="handleSubmit" :loading="loading">
        <v-container>
            ...
            <template v-if="currentStep === 1">
                <v-row>
                    <v-col cols="12">
                        <DuTextField v-model="formData.identifier" label="用户名或邮箱" :rules="identifierRules"
                            prepend-inner-icon="mdi-account" placeholder="请输入用户名或邮箱地址" autofocus required>
                            <template #append-inner>
                                <v-icon v-if="isValidEmail" color="success" size="small">
                                    mdi-check-circle
                                </v-icon>
                            </template>
                        </DuTextField>
                ...
                <v-row>
                    <v-col cols="12" class="text-center">
                        <v-btn color="primary" :loading="loading" :disabled="!isIdentifierValid" @click="handleSendCode"
                            size="large" block>
            ...
            <template v-if="currentStep === 2">
                <v-row>
                    <v-col cols="12">
                        <DuTextField v-model="formData.verificationCode" label="验证码" :rules="codeRules"
                            prepend-inner-icon="mdi-shield-key" placeholder="请输入6位验证码" :counter="6" autofocus required>
                            <template #append-inner>
                                <v-btn variant="text" size="small" color="primary" :disabled="resendCountdown > 0"
                ...
                <v-row>
                    <v-col cols="6">
                        <v-btn variant="outlined" @click="currentStep = 1" :disabled="loading" size="large" block>
                            上一步
                        </v-btn>
                    </v-col>
                    <v-col cols="6">
                        <v-btn color="primary" :loading="loading" :disabled="!isCodeValid" @click="handleVerifyCode"
                            size="large" block>
            ...
            <template v-if="currentStep === 3">
                <v-row>
                    <v-col cols="12">
                        <DuTextField v-model="formData.newPassword" label="新密码" type="password" :rules="passwordRules"
                            prepend-inner-icon="mdi-lock" :show-password-strength="true" required />
                    </v-col>
                </v-row>

                <v-row>
                    <v-col cols="12">
                        <DuTextField v-model="formData.confirmPassword" label="确认新密码" type="password"
                ...
                <v-row>
                    <v-col cols="6">
                        <v-btn variant="outlined" @click="currentStep = 2" :disabled="loading" size="large" block>
                            上一步
                        </v-btn>
                    </v-col>
                    <v-col cols="6">
                        <v-btn color="primary" type="submit" :loading="loading" :disabled="!isPasswordValid"
                            size="large" block>
