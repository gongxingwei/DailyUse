...
import { AccountStatus, AccountType, IAccount, AccountDTO } from '../types';
...
export class Account extends AggregateRoot implements IAccount {
  ...
  verifyEmail(): void {
    ...
    if (!this._phoneNumber || this._isPhoneVerified) {
  ...
  verifyPhone(): void {
    ...
    // 如果邮箱也已验证或不存在，则激活账号
    if (!this._email || this._isEmailVerified) {
      this._status = AccountStatus.ACTIVE;
    }
  ...
  static ensureAccount(account: IAccount | Account | null): Account | null {
    if (Account.isAccount(account)) {
      return account instanceof Account ? account : Account.fromDTO(account as AccountDTO);
    } else {
      return null;
    }
  }
  ...
  static fromDTO(dto: AccountDTO): Account {
    const account = new Account({
      ...
      phoneVerificationCode: dto.phoneVerificationCode,
      roleUuids: dto.roleIds,
    });

    account._createdAt = dto.createdAt;
    account._updatedAt = dto.updatedAt;
