/**
 * 共享事件契约 - Authentication 模块
 * 定义 Authentication 模块发布的所有领域事件
 * 其他模块可以订阅这些事件，但不直接依赖 Authentication 模块
 */

import type { DomainEvent } from '../../domainEvent';

// ================================
// Authentication 查询请求事件（用于模块间通信）
// ================================
...
export interface LoginAttemptEventPayload {
  username: string;
  accountUuid?: string;
  attemptResult: 'success' | 'failed' | 'blocked';
  failureReason?: string;
  attemptedAt: Date;
  clientInfo?: {
    ipAddress?: string;
...
export interface SessionTerminatedEventPayload {
  accountUuid: string;
  sessionId: string;
  terminationReason: 'logout' | 'timeout' | 'security' | 'admin';
  terminatedAt: Date;
}
...
// ================================
// 导出所有 Authentication 相关事件类型
// ================================

export type AuthenticationDomainEvents =
