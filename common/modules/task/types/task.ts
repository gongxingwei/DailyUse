 ...
 * 任务提醒配置 DTO
 */
export type TaskReminderConfigDTO = {
  /** 是否启用提醒 */
 ...
 * 主进程任务模板接口
 */
export interface IMainProcessTaskTemplate extends ITaskTemplate {}

/**
 * 渲染进程任务模板接口
 */
export interface IRenderProcessTaskTemplate extends ITaskTemplate {}
...
export interface ITaskTemplateDTO {
  ...
  /** 调度策略 */
  schedulingPolicy: {
    allowReschedule: number; // 0 或 1
    maxDelayDays: number;
    skipWeekends: number; // 0 或 1
    skipHolidays: number; // 0 或 1
    workingHoursOnly: number; // 0 或 1
  };
  /** 元数据 */
  metadata: {
    category: string;
 ...
 * 任务实例中的提醒状态 DTO
 */
export type TaskInstanceReminderStatusDTO = {
  /** 是否启用提醒 */
  enabled: number; // 0 或 1
  /** 提醒项状态列表 */
  alerts: Array<{
 ...
 * 任务实例时间配置 DTO
 */
export type TaskInstanceTimeConfigDTO = {
  /** 任务类型 */
  type: "allDay" | "timed" | "timeRange";
  /** 计划执行时间 */
  scheduledTime: number;
 ...
 * 主进程任务实例接口
 */
export interface IMainProcessTaskInstance extends ITaskInstance {}

/**
 * 渲染进程任务实例接口
 */
export interface IRenderProcessTaskInstance extends ITaskInstance {}
...
export interface ITaskInstanceDTO {
  ...
  lifecycle: {
    ...
    cancelledAt?: number;
    /** 生命周期事件记录 */
    events: TaskInstanceLifecycleEventDTO[];
  };
  /** 元数据 */
  metadata: {
    estimatedDuration?: number;
 ...
 * 主进程任务元模板接口
 */
export interface IMainProcessTaskMetaTemplate extends ITaskMetaTemplate {}

/**
 * 渲染进程任务元模板接口
 */
export interface IRenderProcessTaskMetaTemplate extends ITaskMetaTemplate {}

/**
 * 任务元模板 DTO
 */
export interface ITaskMetaTemplateDTO {
  /** 元模板ID */
  uuid: string;
  /** 元模板名称 */
  name: string;
  ...
  /** 默认提醒配置 */
  defaultReminderConfig: TaskReminderConfigDTO;
  /** 默认元数据 */
  defaultMetadata: {
    category: string;
    tags: string[];
