...
- [ ] **Day 2 下午**: Application Service
  ...
  export class ScheduleConflictDetectionService {
     ...
     * 检测新日程是否与现有日程冲突
     */
    async detectConflicts(newSchedule: Schedule, userId: string): Promise<ScheduleConflictDTO[]> {
...
- [ ] **下午**: API Endpoints
  ...
  export class ScheduleController {
    ...
    async resolveConflict(
      ...
      body: {
        conflictId: string;
        solution: 'move_forward' | 'move_backward' | 'shorten';
        newStartTime: number;
        newEndTime: number;
      },
    ): Promise<void> {
    ...
    <el-dialog v-model="visible" title="检测到日程冲突">
      <div class="conflicts">
        <div v-for="conflict in conflicts" :key="conflict.conflictId" class="conflict-item">
          ...
          <div class="overlap-info">重叠时长: {{ conflict.overlapDuration }} 分钟</div>
...
- [ ] **Day 6**: Contracts + Domain
  ...
  export class ReminderFrequency extends Entity {
    ...
    private adjustInterval(): void {
      const recentRate =
        this._recentResponses.filter((r) => r).length / this._recentResponses.length;
