...
class WeightRecommendationService {
  ...
  private steppedStrategy(priorities: number[]): WeightStrategy {
    ...
    const weights = sorted.map((_, i) => (priorities.length - i) * step);
...
class AIWeightRecommendationService {
  async recommendWeights(goal: Goal, keyResults: KeyResult[]): Promise<WeightStrategy[]> {
...
<template>
  <v-card class="weight-suggestion-panel">
    ...
    <v-card-subtitle> 基于 KR 内容分析，推荐以下 3 种权重分配策略 </v-card-subtitle>
      ...
      <v-row>
        <v-col v-for="strategy in strategies" :key="strategy.name" cols="12" md="4">
          ...
          >
            ...
            <v-card-actions>
              <v-btn block color="primary" variant="tonal" @click="applyStrategy(strategy)">
                应用此策略
              </v-btn>
            </v-card-actions>
          </v-card>
    ...
    <v-card-actions>
      ...
      <v-btn color="primary" :disabled="!selectedStrategy" @click="confirmSelection">
        确认选择
      </v-btn>
    </v-card-actions>
  </v-card>
...
describe('WeightRecommendationService', () => {
  it('should generate balanced strategy', () => {
    const krs = [{ title: 'KR 1' }, { title: 'KR 2' }, { title: 'KR 3' }];
