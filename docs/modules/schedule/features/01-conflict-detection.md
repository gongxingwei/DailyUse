 ...
 * 检测两个日程是否冲突
 */
function detectConflict(scheduleA: Schedule, scheduleB: Schedule): ScheduleConflict | null {
...
function generateResolutions(
  ...
  // 方案 2: 取消低优先级冲突日程
  const lowerPrioritySchedules = conflicts.filter((c) => c.priority < schedule.priority);
